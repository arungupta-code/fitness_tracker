...
const TotalCaloriesChart = ({ data }) => {
  ...
  const totalCalories = aligned.reduce((acc, value, index) => {
    const prev = index === 0 ? 0 : acc[index - 1];
    acc.push(prev + Number(value || 0));
    return acc;
  }, []);

  return (
    <Card>
      <Title>Total Calories Burned (Cumulative)</Title>

      {!data && <Hint>Loading chart…</Hint>}

      {data && totalCalories.every((v) => v === 0) && (
        <Hint>No calories burned in last 7 days.</Hint>
      )}
      ...
      <LineChart
        ...
        series={[
          {
            data: totalCalories,   // ✅ cumulative data
            label: "Total",
            area: true,
            curve: "monotoneX",
          },
